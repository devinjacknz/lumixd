...
async function executeSwap(walletKey) {
    try {
        ...
        const quoteResponse = await axios.get(`${JUPITER_V6_ENDPOINT}/quote`, { params: quoteParams });
        const quote = quoteResponse.data;
        
        if (!validateRequestData(quote)) {
        ...
        if (!validateRequestData(swapResult)) {
        ...
        if (!validateRequestData(executeResult)) {
