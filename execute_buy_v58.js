...
async function executeSwap(walletKey) {
    try {
        ...
        // Pass the quote response directly without modification
        const swapRequest = {
            quoteResponse: quote,
        ...
        console.log('\nSwap Request:', JSON.stringify(swapRequest, null, 2));
        
        const swapResponse = await axios.post(`${JUPITER_V6_ENDPOINT}/swap`, swapRequest);
        
        if (!swapResponse.data || !swapResponse.data.swapTransaction) {
            console.error('Swap Response:', JSON.stringify(swapResponse.data, null, 2));
