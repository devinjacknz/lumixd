...
const executeSwap = async (walletKey) => {
    try {
        ...
        const quoteUrl = `${JUPITER_V6_ENDPOINT}/quote?inputMint=${INPUT_MINT}&outputMint=${OUTPUT_MINT}&amount=${AMOUNT}&slippageBps=${SLIPPAGE_BPS}&onlyDirectRoutes=false`;
        ...
        if (swapResult.error) {
            throw new Error(`Swap error: ${swapResult.error}`);
        }
        ...
        if (executeResult.error) {
            throw new Error(`Execute error: ${executeResult.error}`);
        }
