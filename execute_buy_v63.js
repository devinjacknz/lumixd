...
async function executeSwap(walletKey) {
    try {
        ...
        // Use swap-instructions endpoint with exact format from docs
        ...
        console.log('\nSwap Instructions Request:', JSON.stringify(swapInstructionsRequest, null, 2));
        
        const swapInstructionsResponse = await axios.post(
            `${JUPITER_V6_ENDPOINT}/swap-instructions`,
            swapInstructionsRequest
        );
        
        if (!swapInstructionsResponse.data || !swapInstructionsResponse.data.swapTransaction) {
            console.error('Swap Instructions Response:', JSON.stringify(swapInstructionsResponse.data, null, 2));
        ...
        const executeResponse = await axios.post(`${JUPITER_V6_ENDPOINT}/swap/execute`, {
            swapTransaction: swapInstructionsResponse.data.swapTransaction,
            userPublicKey: keypair.publicKey.toString(),
            dynamicComputeUnitLimit: true
        });
        
        if (!executeResponse.data || !executeResponse.data.txid) {
            console.error('Execute Response:', JSON.stringify(executeResponse.data, null, 2));
            throw new Error('Invalid execute response');
        }
        
        const signature = executeResponse.data.txid;
