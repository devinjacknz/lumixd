...
SLIPPAGE_BPS = int(os.getenv('DEFAULT_SLIPPAGE_BPS', '250'))
...
POSITION_SIZE = 5_000_000_000  # 5 SOL in lamports
...
async def get_swap_instructions(quote_response: Dict, wallet_key: str) -> Optional[Dict]:
...
async def execute_swap(swap_response: Dict, wallet_key: str) -> Optional[str]:
    ...
    try:
        ...
        url = f"{JUPITER_API_URL}/swap/execute"
        execute_data = {
...
async def verify_transaction(signature: str) -> bool:
    ...
    try:
        ...
        for attempt in range(MAX_RETRIES):
            ...
            print(f"\n⏳ 等待交易确认... | Waiting for confirmation... (attempt {attempt + 1}/{MAX_RETRIES})")
